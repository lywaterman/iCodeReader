...
namespace LibGit2Sharp
{
    ...
    public class Commit : GitObject
    {
		...
		private readonly LibGit2Sharp.Core.Compat.Lazy<string> lazyShortMessage;
		private readonly LibGit2Sharp.Core.Compat.Lazy<IEnumerable<Note>> lazyNotes;
            ...
            : base(repo, id)
        {
			...
			lazyShortMessage = new LibGit2Sharp.Core.Compat.Lazy<string>(ExtractShortMessage);
			lazyNotes = new LibGit2Sharp.Core.Compat.Lazy<IEnumerable<Note>>(() => RetrieveNotesOfCommit(id).ToList());
        ...
        {
			private readonly LibGit2Sharp.Core.Compat.Lazy<ICollection<Commit>> _parents;
			private readonly LibGit2Sharp.Core.Compat.Lazy<int> _count;

            public ParentsCollection(Repository repo, ObjectId commitId)
            {
				_count = new LibGit2Sharp.Core.Compat.Lazy<int>(() => Proxy.git_commit_parentcount(repo.Handle, commitId));
				_parents = new LibGit2Sharp.Core.Compat.Lazy<ICollection<Commit>>(() => RetrieveParentsOfCommit(repo, commitId));
            }
