...
namespace LibGit2Sharp
{
    ...
    public class TreeEntry : IEquatable<TreeEntry>
    {
		...
		private readonly LibGit2Sharp.Core.Compat.Lazy<GitObject> target;
        private readonly ObjectId targetOid;
		private readonly LibGit2Sharp.Core.Compat.Lazy<string> path;
        ...
        internal TreeEntry(SafeHandle obj, ObjectId parentTreeId, Repository repo, FilePath parentPath)
        {
            ...
            target = new LibGit2Sharp.Core.Compat.Lazy<GitObject>(RetrieveTreeEntryTarget);

            Mode = Proxy.git_tree_entry_attributes(obj);
            Name = Proxy.git_tree_entry_name(obj);
            path = new LibGit2Sharp.Core.Compat.Lazy<string>(() => System.IO.Path.Combine(parentPath.Native, Name));
        }
