...
namespace LibGit2Sharp
{
    ...
    public class Index : IEnumerable<IndexEntry>
    {
        ...
        public virtual void Move(IEnumerable<string> sourcePaths, IEnumerable<string> destinationPaths)
        {
			...
			IDictionary<LibGit2Sharp.Core.Compat.Tuple<string, FileStatus>, LibGit2Sharp.Core.Compat.Tuple<string, FileStatus>> batch = PrepareBatch(sourcePaths, destinationPaths);
			...
			foreach (KeyValuePair<LibGit2Sharp.Core.Compat.Tuple<string, FileStatus>, LibGit2Sharp.Core.Compat.Tuple<string, FileStatus>> keyValuePair in batch)
            {
		...
		private IDictionary<LibGit2Sharp.Core.Compat.Tuple<string, FileStatus>, LibGit2Sharp.Core.Compat.Tuple<string, FileStatus>> PrepareBatch(IEnumerable<string> leftPaths, IEnumerable<string> rightPaths)
        {
            IDictionary<LibGit2Sharp.Core.Compat.Tuple<string, FileStatus>, LibGit2Sharp.Core.Compat.Tuple<string, FileStatus>> dic = new Dictionary<LibGit2Sharp.Core.Compat.Tuple<string, FileStatus>, LibGit2Sharp.Core.Compat.Tuple<string, FileStatus>>();
            ...
            {
                LibGit2Sharp.Core.Compat.Tuple<string, FileStatus> from = BuildFrom(leftEnum.Current);
                LibGit2Sharp.Core.Compat.Tuple<string, FileStatus> to = BuildFrom(rightEnum.Current);
                dic.Add(from, to);
            }
		...
		private LibGit2Sharp.Core.Compat.Tuple<string, FileStatus> BuildFrom(string path)
        {
            string relativePath = repo.BuildRelativePathFrom(path);
            return new LibGit2Sharp.Core.Compat.Tuple<string, FileStatus>(relativePath, RetrieveStatus(relativePath));
        }
